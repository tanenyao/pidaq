...
class AudioStream(object):
    ...
    def set_plotdata(self, name, data_x, data_y):
	    ...
	    if name == 'peaks':
                self.traces[name] = self.spectrum.plot(pen=None, symbol='o')

    def update(self):
	# get values from MCC118
	...
	# removes error whereby len(wf_data.data)=0 ---------- probably due to high refresh rates
	if len(wf_data.data) == self.sample_size:
		...
		self.set_plotdata(name='peaks', data_x=[j.item()/float(self.sample_size)*self.rate  for j in indexes], data_y=sp_data[indexes])
		...
		if len(indexes) != 0:
			for i in indexes:
				...
				# convert indexes to spectrum values
				x = i.item() / float(self.sample_size) * self.rate
				tup = (round(x - range,2), round(x + range, 2))
				fl.append(tup)
    ...
    def p_range(self, x, ratio):
	return (round(x-x*ratio,2), round(x+x*ratio,2))
